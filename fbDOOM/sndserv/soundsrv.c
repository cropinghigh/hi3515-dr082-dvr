...
int mix(void)
{
	...
	for(int i = 0; i < period_size; i+=CHNUM) {
		dcomb = 0;
		...
		for(int ch = 0; ch < 8; ch++) {
			...
			{
				sample = *channels[ch];
				dcomb += (channelleftvol_lookup[ch][sample]+channelrightvol_lookup[ch][sample])/2;
		...
		if(CHNUM == 1) {
			if (dcomb > 0x7fff)
				mixbuffer[i] = 0x7fff;
			else if (dcomb < -0x8000)
				mixbuffer[i] = -0x8000;
			else
				mixbuffer[i] = dcomb;
		} else {
			for(int z = 0; z < CHNUM; z++) {
				if (dcomb > 0x7fff)
					mixbuffer[i+z] = 0x7fff;
				else if (dcomb < -0x8000)
					mixbuffer[i+z] = -0x8000;
				else
					mixbuffer[i+z] = dcomb;
			}
		}
...
void updatesounds(void)
{
    ...
    mix();
    I_SubmitOutputBuffer(mixbuffer, period_size);
  ...
  int		seperation )
{
	...
	// printf("ADDDED CH%d: VOL %d; %d, STEP %d, HANDLE %d, REMAINDER %d\n", slot, leftvol*256, rightvol*256, step, rc, channelstepremainder[slot]);
...
void initdata(void)
{
    ...
    uint32_t*	steptablemid = &steptable[128];
  ...
  char**	v )
{
	...
	I_InitSound(16000, 16);
	
	mixbuffer = malloc((period_size * CHNUM * snd_pcm_format_physical_width(FMT)) / 8);
    if (mixbuffer == NULL) {
