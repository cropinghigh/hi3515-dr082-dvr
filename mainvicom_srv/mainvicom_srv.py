...
def start_ffmpeg_av_proc(source, dest):
    ...
    if(ffmpeg_v_proc == -1):
        ffmpeg_v_proc = []
        ffmpeg_v_proc_a = subprocess.Popen(["bash","-c","ffmpeg -re -probesize 65536 -analyzeduration 65536 -fflags nobuffer -flags low_delay -i " + source + " -r 6 -vcodec mpeg2video -filter:v \"scale=640x480\" -bufsize 128k -maxrate 4000k -qscale:v 2 -fflags nobuffer -flags low_delay -avioflags direct -an -f mpegts udp://" + dest + ":8101?pkt_size=1024 -vn -filter:a \"volume=" + volume_downlink + "\" -acodec pcm_s16le -ac 1 -ar 8000 -f s16le udp://127.0.0.1:8105?pkg_size=1024"], preexec_fn=os.setsid)
        ffmpeg_v_proc_b = subprocess.Popen(["bash","-c","ffmpeg -re -probesize 32768 -analyzeduration 32768 -fflags nobuffer -flags low_delay -f s16le -ac 1 -ar 8000 -i udp://127.0.0.1:8103?listen=1 -fflags nobuffer -flags low_delay -avioflags direct -acodec pcm_s16le -ac 1 -ar 8000 -f s16le udp://" + dest + ":8104?pkt_size=1024"], preexec_fn=os.setsid)
...
def start_ffmpeg_a_proc_fromvideo(source, dest):
    ...
    if(ffmpeg_a_proc == -1):
        ffmpeg_a_proc = 1
        ffmpeg_a_proc = subprocess.Popen(["bash","-c","ffmpeg -re -probesize 32768 -analyzeduration 32768 -fflags nobuffer -flags low_delay -f s16le -ac 1 -ar 8000 -i udp://127.0.0.1:8105?listen=1 -fflags nobuffer -flags low_delay -avioflags direct -acodec pcm_s16le -ac 1 -ar 8000 -f s16le udp://127.0.0.1:8103?pkt_size=1024"], preexec_fn=os.setsid)
...
def start_ffmpeg_a_proc_fromfile(source, dest):
    ...
    if(ffmpeg_a_proc == -1):
        ffmpeg_a_proc = 1
        ffmpeg_a_proc = subprocess.Popen(["bash","-c","ffmpeg -re -stream_loop -1 -i " + source + " -fflags nobuffer -flags low_delay -avioflags direct -acodec pcm_s16le -ac 1 -ar 8000 -f s16le udp://127.0.0.1:8103?pkt_size=1024"], preexec_fn=os.setsid)
