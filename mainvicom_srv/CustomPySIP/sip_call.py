...
from .utils import get_caller_number
...
class SipCall:
    ...
    async def start(self):
        # async with self.call_lock:
            ...
            _tasks = []
            try:
                self.my_public_ip = await asyncio.to_thread(self.sip_core.get_public_ip, self.server)
                self.my_private_ip = await asyncio.to_thread(self.sip_core.get_local_ip, self.server)
    ...
    async def handle_incoming_call_delayed(self, initial_invite):
        try:
            response = await asyncio.wait_for(self.call_response_future, 30.0)
            if(response != -1):
    ...
    async def handle_incoming_call(self, initial_invite: SipMessage):
        # async with self.call_lock:
            ...
            if(initial_invite.get_header("m")):
                self.unknown_m = initial_invite.get_header("m")
                
            if(initial_invite.get_header("Record-Route")):
                self.route = initial_invite.get_header("Record-Route")
            ...
            delayed_task = asyncio.create_task(
                        self.handle_incoming_call_delayed(initial_invite), name="Delayed incoming handle Task"
                    )
    ...
    def setup_local_session(self):
        sdp = SipMessage.generate_sdp(
            self.sip_core.get_local_ip(self.server),
    ...
    def bye_generator(self):
        ...
        if(self.route):
            msg += f"Route: {self.route}\r\n"
        msg += f"User-Agent: Myhome/Myhome-android\r\n"
        msg += "Content-Length: 0\r\n\r\n"
    ...
    def ok_generator(self, data_parsed: SipMessage, include_sdp=False):
        ...
        if include_sdp:
            sdp = SipMessage.generate_sdp(
                self.sip_core.get_public_ip(self.server),
                self.dialogue.local_session_info.port,
                random.getrandbits(32),
                CODECS,
                self.username
            )
        ...
        if data_parsed.is_from_client(self.username):  # outgoing call
            ...
            from_field, to_field = data_parsed.get_header("From"), data_parsed.get_header("To")
            if(not from_field):
                from_field = data_parsed.get_header("f")
            if(not to_field):
                to_field = data_parsed.get_header("t")
        ...
        msg = "SIP/2.0 200 Ok\r\n"
        via = data_parsed.get_header('Via')
        via2 = data_parsed.get_header('v')
        if(via):
            msg += f"Via: {via}\r\n"
        if(via2):
            msg += f"Via: {via2}\r\n"
        ...
        msg += f"Contact: <sip:{self.username}@{self.server};gr=urn:uuid:8aaaaaaa-aaaa-aaaa-aaaa-9962021adc65;transport={self.CTS.upper()};ob>;+sip.instance=\"<urn:uuid:8aaaaaaa-aaaa-aaaa-aaaa-9962021adc65>\"\r\n"
        msg += f"Allow: {', '.join(SIPCompatibleMethods)}\r\n"
        msg += f"User-Agent: Myhome/Myhome-android\r\n"
        msg += "Supported: replaces, outbound, gruu, path\r\n"
        msg += "Content-Type: application/sdp\r\n" if include_sdp else ""
        if(self.route):
            msg += f"Record-Route: {self.route}\r\n"
    ...
    def generate_trying_response(self, data_parsed: SipMessage) -> str:
        ...
        if(via2):
            msg += f"Via: {via2}\r\n"
        fr = data_parsed.get_header("From")
        if(not fr):
            fr = data_parsed.get_header("f")
        to = data_parsed.get_header("To")
        if(not to):
            to = data_parsed.get_header("t")
        msg += f"From: {fr}\r\n"
        msg += f"To: {to}\r\n"  # No tag in Trying
        msg += f"Call-ID: {data_parsed.call_id}\r\n"
    ...
    def generate_ringing_response(self, invite_message: SipMessage) -> str:
        ...
        via = invite_message.get_header("Via")
        if(not via):
            via = invite_message.get_header("v")
        fr = invite_message.get_header("From")
        if(not fr):
            fr = invite_message.get_header("f")
        to = invite_message.get_header("To")
        if(not to):
            to = invite_message.get_header("t")
        msg += "Via: " + via + "\r\n"
        msg += f"From: {fr}\r\n"
        msg += (
    ...
    async def error_handler(self, msg: SipMessage):
        if(msg.method == "CANCEL"):
            if self.call_response_future and not self.call_response_future.done():
                self.call_response_future.set_result(-1)
                ok_message = self.ok_generator(msg)
                await self.sip_core.send(ok_message)
                await self.stop("Callee cancelled call")
                return
