...
class RTPClient:
    def __init__(
        self, offered_codecs, src_ip, src_port, dst_ip, dst_port, transmit_type, ssrc, out_volume = 1.0,
    ...
    async def _stop(self):
        # print("STOPPING RTP SESSION")
        self.is_running.clear()
        # print("STOPPING RTP SESSION 1")
        self.__rtp_socket.close() 
        # print("STOPPING RTP SESSION 2")
        ...
        for t in self.__all_threads:
            # print("Stopping " + str(t.name))
            await asyncio.to_thread(t.join)
            
        # print("STOPPING RTP SESSION5")
    ...
    def send(self, loop: asyncio.AbstractEventLoop):
        while True:
            ...
            encoded_payload = self.__encoder.encode(payload, self.out_volume)
            packet = RtpPacket(
        ...
        while True:
            ...
            try: 
                ...
                if encoded_frame:
                    ...
                    decoded_frame = self.__decoder.decode(encoded_frame.data, 1.0)
                    for output_q in self._output_queues.values():
                        if isinstance(output_q, asyncio.Queue):
                            # asyncio.run_coroutine_threadsafe(output_q.put(decoded_frame), loop)
                            try:
