...
class SipClient:
    ...
    async def run(self):
        ...
        tasks = []
        try:
            self.my_public_ip = await asyncio.to_thread(self.sip_core.get_public_ip, self.server.split(":")[0])
            self.my_private_ip = await asyncio.to_thread(self.sip_core.get_local_ip, self.server.split(":")[0])
    ...
    def build_register_message(
        ...
        if auth:
            ...
            expires_field = "Expires: " + str(self.register_duration) + "\r\n" if not unregister else ""
            ...
            msg = (
                ...
                f"User-Agent: Myhome/Myhome-android\r\n"
                f"Contact: <sip:{self.username}@{ip}:{port};transport={self.CTS};app-id=erth;Call-Id:%20null>{expires}\r\n"
                f"{expires_field}"
    ...
    async def message_handler(self, msg: SipMessage):
        ...
        elif(msg.data.startswith("NOTIFY")):
            logger.log(
                logging.DEBUG, "Notify message received from the server. sending OK"
            )
            notify_ok = self.ok_generator(msg)
            await self.sip_core.send(notify_ok)
