...
class SipCore:
    ...
    def get_public_ip(self,srvr) -> str | None:
        # try:
        ...
        # 
        # return external_ip
        return self.get_local_ip(srvr)

    def get_local_ip(self, srvr):
        try:
    ...
    async def send_to_callbacks_d(self, data):
        for callback in self.on_message_callbacks:
            data_formatted = SipMessage(data)
            # with ThreadPoolExecutor() as executor:
            #     loop = asyncio.get_event_loop()
    ...
    async def receive(self):
        while True:
            ...
            for sip_message_data in sip_messages:
                await self.send_to_callbacks_d(sip_message_data.decode())
    ...
    def extract_sip_messages(self, data: bytes) -> List[bytes]:
        ...
        while start < len(data):
            ...
            if(len(content_length) < 1):
                print("INVALID PACKET(LEN NOT FOUND): " + str(headers))
...
class SipDialogue:
    ...
    def update_state(self, message):
        ...
        if (
            ...
            self.state in [DialogState.INITIAL, DialogState.EARLY, DialogState.PREDIALOG] and is_final_response
        ):
            # print("CFRMD")
...
class SipMessage:
    ...
    def set_properties(self):
        ...
        if(cseq == None):
            print("INVALID PACKET(NO Cseq): " + str(self.data))
        ...
        if(not from_header):
            from_header = self.get_header("f")
        if(not from_header):
            print("INVALID PACKET(NO from): " + str(self.data))
            return #invalid packet
    ...
    @classmethod
    def generate_sdp(cls, ip, port, ssrc, supported_codecs, user="-"):
        cname = f"host_{random.randint(100, 999)}"

        sdp = f"v=0\r\n"
        sdp += f"o={user} {random.randint(100000000, 999999999)} {random.randint(100000000, 999999999)} IN IP4 {ip}\r\n"
        sdp += "s=Talk\r\n"
        sdp += f"c=IN IP4 {ip}\r\n"
        sdp += "t=0 0\r\n"
